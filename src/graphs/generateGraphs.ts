...
import { createStackedBar, type StackedDataPoint } from "./stackedBar";
import { createSpeed } from "./speed";
import { createWellCols } from "./wellCols";
import { createSankey } from "./sankey";
import { createRadar, type radarAxis } from "./radar";
import { createOverflowStackedBar } from "./overflowStackedBar";
import { createMultiWellCols } from "./multiWellCols";
import { createMultiPPS } from "./ppsSegments";

import { defaultRainbow } from "@/theme/colors";

import * as d3 from "d3"

export function createGraphs(rootDiv: HTMLElement, stats: { [key: string]: CumulativeStats }): string[] {
  const graphs: string[] = []
  {
...

  {
    ...
    stacked.push(createOverflowStackedBar<StatMap>(rootDiv, "", data, 90, keys, colors, shortName, 30))
  }
...

  {
    ...
    stacked.push(createOverflowStackedBar<StatMap>(rootDiv, "PPS", data, 2.5 * 3, keys, colors, shortName, 0.9))
  }
...

  {
    ...
    stacked.push(createOverflowStackedBar<StatMap>(rootDiv, "APM", data, 300, keys, colors, shortName, 37.5))
  }
...

  {
    ...
    speeds.push(createSpeed(rootDiv, "Downstacking Ratio", data, names, 100, [0, 20, 40, 60, 80, 100], labels))
  }

  {
    ...
    speeds.push(createSpeed(rootDiv, "Attack Cheesiness", data, names, 100, [0, 20, 40, 60, 80, 100], labels))
  }
